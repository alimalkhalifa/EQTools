...
export default class ZoneFileLoader {
  ...
  private static loadPFS(buffer: Buffer): [S3DFile[], Buffer] {
    ...
    if (directory_length !== file_list.length) {
    ...
    for (let i = 0; i < fragment_count; i++) {
      ...
      switch (fragment_type) {
        case 0x03: // Texture Path
          texture_paths.push(this.loadTexturePath(i, fragment_name, reader.read(fragment_size - 4), files));
          break;
        case 0x04: // Texture Info
          texture_infos.push(this.loadTextureInfo(i, fragment_name, reader.read(fragment_size - 4)));
          break;
        case 0x05: // Texture Info Ref
          texture_info_refs.push(this.loadTextureInfoRef(i, fragment_name, reader.read(fragment_size - 4)));
          break;
        case 0x14: // Static Mesh
          static_meshes.push(this.loadStaticMesh(i, fragment_name, reader.read(fragment_size - 4)));
          break;
        ...
        case 0x2D: // Mesh Reference
          mesh_references.push(this.loadMeshReference(i, fragment_name, reader.read(fragment_size - 4), meshes));
          break
        case 0x30: // Texture
          materials.push(this.loadTextureFragment(i, fragment_name, reader.read(fragment_size - 4), texture_info_refs, texture_infos, texture_paths));
          break;
        case 0x31: // Texture List
          texture_lists.push(this.loadTextureList(i, fragment_name, reader.read(fragment_size - 4)));
          break;
        case 0x36: // Mesh
          meshes.push(this.loadMeshFragment(i, fragment_name, reader.read(fragment_size - 4), texture_lists, materials));
          break;
        default:
  ...
  private static loadMeshFragment(id: number, fragment_name: string, buffer: Buffer, texture_lists: { id: number, name: string, materials: number[] }[], materials: MaterialInterface[]): MeshInterface {
    ...
    const material_0 = materials.find((value) => value.id === materials_list[0]);
    ...
    for (let i = 0; i < texture_cooordinate_count; i++) {
      texture_coordinates.push({
        x: reader.readInt16() / (material_0.width * 2.0),
        y: reader.readInt16() / (material_0.height * 2.0),
      });
    }
  ...
  private static loadTextureFragment(
    ...
    const texture_info = texture_infos.find((value) => value.id === texture_info_ref.ref);
    const textures: string[] = [];
    for (const ref of texture_info.refs) {
      textures.push(texture_paths.find((value) => value.id === ref).file_path);
    }
  ...
  private static loadMeshReference(id: number, fragment_name: string, buffer: Buffer, meshes: MeshInterface[]): MeshReferenceInterface {
    ...
    let name = meshes.find(value => value.id === reference).name
    return {
      id,
      name,
      reference
